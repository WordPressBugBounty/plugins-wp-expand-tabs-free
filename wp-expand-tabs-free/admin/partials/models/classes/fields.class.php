...
if ( ! class_exists( 'SP_WP_TABS_Fields' ) ) {
	...
	abstract class SP_WP_TABS_Fields extends SP_WP_TABS_Abstract {
		 ...
		 *
		 * @param  int   $parent_cat_id parent cat id.
		 * @param  array $options options.
		 * @return mixed
		 */
		public static function get_child_categories( $parent_cat_id = 0, $options = '' ) {
			...
			if ( ! is_wp_error( $child_categories ) && ! empty( $child_categories ) ) {
				foreach ( $child_categories as $child_category ) {
					$ancestor         = count( get_ancestors( $child_category->term_id, 'product_cat', 'taxonomy' ) );
					$current_ancestor = 1;
					$hyphen           = '';
					...
					$options[ $child_category->term_id ] = $hyphen . $child_category->name . ' (' . $child_category->count . ')';
					$options                             = self::get_child_categories( $child_category->term_id, $options );
				}
			}
			return $options;
		}
		...
		public static function field_data( $type = '', $term = false, $query_args = array() ) {
			...
			switch ( $type ) {
				...
				case 'term':
					...
					break;
				case 'sp_wp_tabs_subcategories':
				case 'sp_wp_tabs_subcategory':
					$options = self::get_child_categories( 0, $options );
					break;
				case 'shortcode_list':
					$wp_tab_get_specific = array(
						'post_type' => 'sp_wp_tabs',
					);
					$query_args          = array_merge( $query_args, $wp_tab_get_specific );
					$all_posts           = get_posts( $query_args );
