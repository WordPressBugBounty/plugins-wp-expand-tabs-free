	...
	class SP_WP_TABS_Options extends SP_WP_TABS_Abstract {
		...
		public function add_options_html() {
			...
			if ( $show_buttons && $smart_tabs_button ) {
				echo '<h1><img src="' . esc_attr( Constants::cs_framework_assets() . '/images/smart-tabs-logo.svg' ) . '" alt="">' . esc_html( $this->args['framework_title'] ) . '</h1>';
			} elseif ( $show_buttons ) {
				echo '<h1><img src="' . esc_attr( Constants::cs_framework_assets() . '/images/tab-icon.svg' ) . '" alt="">' . esc_html( $this->args['framework_title'] ) . '</h1>';
			} else {
				echo '<h1><img src="' . esc_attr( Constants::cs_framework_assets() . '/images/import-export.svg' ) . '" alt="">' . esc_html( $this->args['framework_title'] ) . '</h1>';
			}
			...
			foreach ( $this->pre_sections as $section ) {
				...
				if ( ! empty( $section['fields'] ) ) {
					...
					foreach ( $section['fields'] as $field ) {
						...
						SP_CSFramework::field( $field, $value, $this->unique, 'options' );
					}
				} else {
